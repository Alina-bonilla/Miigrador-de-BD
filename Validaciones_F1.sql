...
BEGIN
    ...
    UPDATE dbo.Perfil SET Codigo = dbo.ufn_MIG_TRIM(Codigo), Nombre = dbo.ufn_MIG_TRIM(Nombre)
    OUTPUT INSERTED.IdPerfil INTO @Ajustes
    WHERE Codigo <> dbo.ufn_MIG_TRIM(Codigo) OR Nombre <> dbo.ufn_MIG_TRIM(Nombre);
...
BEGIN
    ...
    FROM dbo.Perfil WHERE LEN(Codigo) = 0 OR Codigo IS NULL OR LEN(Nombre) = 0 OR Nombre IS NULL;

    -- Regla 3: longitud > 255
    INSERT INTO dbo.Error (IdTabla, IdRegistro, Codigo, Descripcion)
    SELECT @IdTabla, IdPerfil, 3, 'Campo de texto mayor a 255 caracteres'
    FROM dbo.Perfil WHERE LEN(Codigo) > 255 OR LEN(Nombre) > 255;
